...
use async_std::{fs::File, path::{Path, PathBuf}};
...
pub async fn resolve_lib(root_path: PathBuf, span: &Span, libs: &mut HashMap<PathBuf, Database>, symbol_table: SymbolTable, reporter: &Reporter) -> Database {
    ...
    let namespace = match syntax::parse::parse(&lexed.stream, lexed.source, lexed.lookup, reporter.clone(), &lexed.symbol_table, root_path).await {
